      ...
      <section className="section-space">
        <div className="container-shell grid gap-4">
          {services.map((service) => (
            <Card key={service.id} className="border-white/12 bg-zinc-950/60 p-6 sm:p-7">
              ...
              <div className="mt-6 grid gap-5 lg:grid-cols-2">
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-zinc-300">{service.whatItIs}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-200">Who it is for</p>
                    <p className="mt-2 text-sm text-zinc-300">{service.whoItsFor}</p>
                  </div>
                </div>
                <div className="space-y-4">
                  <div>
                    ...
                    <ul className="mt-2 grid gap-2">
                      {service.deliverables.map((item) => (
                        <li key={item} className="inline-flex items-start gap-2 text-sm text-zinc-300">
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {service.stack.map((item) => (
                        <span key={item} className="rounded-md border border-white/12 bg-white/[0.02] px-2.5 py-1.5 text-xs text-zinc-200">
                          {item}
                        </span>
                      ))}
                    </div>
