...
export function Footer() {
  return (
    <footer className="relative border-t border-white/10 bg-slate-950/65">
      <div className="container-shell grid gap-10 py-14 lg:grid-cols-[1.2fr_1fr_1fr]">
        <div className="space-y-4">
          ...
          <p className="max-w-sm text-sm text-slate-300">{siteConfig.tagline}</p>
        ...
        <div>
          <p className="mb-4 text-sm font-semibold uppercase tracking-[0.18em] text-slate-400">Navigation</p>
          <ul className="space-y-2">
            {navItems.map((item) => (
              <li key={item.href}>
                <Link href={item.href} className="inline-flex items-center gap-2 text-sm text-slate-300 hover:text-white">
                  {item.label}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm text-slate-300">
            ...
            <li>
              <Link
                href="/contact"
                className="inline-flex items-center gap-2 rounded-lg border border-white/15 px-3 py-2 text-sm font-medium text-slate-100 hover:bg-white/10"
              >
                Start Your Project
      ...
      <div className="border-t border-white/10 py-4">
        <div className="container-shell flex flex-col gap-1 text-xs text-slate-400 sm:flex-row sm:items-center sm:justify-between">
